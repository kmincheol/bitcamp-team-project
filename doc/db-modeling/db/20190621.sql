...
-- 인증키
DROP TABLE IF EXISTS atk RESTRICT;
...
CREATE TABLE tm (
  ...
  tm_lev_id    INTEGER(30)  NOT NULL DEFAULT 1 COMMENT '팀수준번호', -- 팀수준번호
  spt_clsf_id  INTEGER(30)  NOT NULL COMMENT '종목유형번호', -- 종목유형번호
...
CREATE TABLE mbr (
  ...
  bth_dt     DATETIME     NULL     COMMENT '생년월일', -- 생년월일
  gndr       CHAR(2)      NULL     DEFAULT "무" COMMENT '성별', -- 성별
  tel        VARCHAR(30)  NULL     COMMENT '휴대폰번호', -- 휴대폰번호
  ...
  main_tm_id INTEGER(30)  NULL     COMMENT '대표팀번호' -- 대표팀번호
)
COMMENT '회원';
...
CREATE TABLE toplc (
  ...
  toplc_name VARCHAR(50) NOT NULL COMMENT '이름' -- 이름
)
COMMENT '시도지역';
...
CREATE TABLE midlc (
  ...
  midlc_name VARCHAR(50) NOT NULL COMMENT '이름', -- 이름
  toplc_id   INTEGER(30) NOT NULL COMMENT '시도지역번호' -- 시도지역번호
)
COMMENT '시군구지역';
...
-- 인증키
CREATE TABLE atk (
  atk_id    INTEGER(30) NOT NULL COMMENT '인증키번호', -- 인증키번호
  atk_email VARCHAR(40) NOT NULL COMMENT '인증이메일', -- 인증이메일
  atk_cont  VARCHAR(50) NOT NULL COMMENT '인증키내용', -- 인증키내용
  atk_type  INTEGER(10) NOT NULL COMMENT '인증타입', -- 인증타입
  atk_dt    DATETIME    NULL     DEFAULT now() COMMENT '생성일' -- 생성일
)
COMMENT '인증키';

-- 인증키
ALTER TABLE atk
  ADD CONSTRAINT PK_atk -- 인증키 기본키
    PRIMARY KEY (
      atk_id -- 인증키번호
    );
...
CREATE TABLE terms_optn (
  ...
  mkt           BOOLEAN     NOT NULL COMMENT '마케팅', -- 마케팅
...
ALTER TABLE terms_optn
  ADD CONSTRAINT PK_terms_optn -- 약관선택 기본키
    PRIMARY KEY (
      terms_optn_id -- 약관번호
    );
...
CREATE TABLE sms (
  ...
  sms_tel  VARCHAR(30) NOT NULL COMMENT '전화번호', -- 전화번호
  sms_cont VARCHAR(50) NOT NULL COMMENT '문자인증키내용', -- 문자인증키내용
  sms_type INTEGER(10) NOT NULL COMMENT '문자인증타입', -- 문자인증타입
  sms_dt   DATETIME    NULL     DEFAULT now() COMMENT '생성일' -- 생성일
)
COMMENT '문자인증';

-- 문자인증
ALTER TABLE sms
  ADD CONSTRAINT PK_sms -- 문자인증 기본키
    PRIMARY KEY (
      sms_id -- 문자인증번호
    );
...
-- 회원
ALTER TABLE mbr
  ADD CONSTRAINT FK_tm_TO_mbr -- 팀 -> 회원
    FOREIGN KEY (
      main_tm_id -- 대표팀번호
    )
    REFERENCES tm ( -- 팀
...
-- 약관선택
ALTER TABLE terms_optn
  ADD CONSTRAINT FK_mbr_TO_terms_optn -- 회원 -> 약관선택
    FOREIGN KEY (
      mbr_id -- 회원번호
    )
    REFERENCES mbr ( -- 회원
...
-- 포지션
ALTER TABLE position
  ADD CONSTRAINT FK_spt_clsf_TO_position -- 종목유형 -> 시군구지역2
    FOREIGN KEY (
      spt_clsf_id -- 종목유형번호
    )
    REFERENCES spt_clsf ( -- 종목유형
