  ...
  <div id="main-wrap" class="container" style="box-shadow: 0px 0px 10px 0px darkgrey; padding: 15px; margin-bottom: 100px;">
    ...
    <form action='update' method='post' name="remove">
      <div id="title-wrap" class="form-group">    
        ...
        <div id="teamName-row" class="col-sm-12">
          <span id="teamName" name="member">${question.member.id}</span> <span>|</span> <span
            id="createdDate" name="createdDate">${question.createdDate}</span>
        </div>
      </div>
    ...
    <div style="width: 100%; text-align: center; margin-top: 30px; margin-bottom: 30px;">
      <div style="display: inline-block;">
        ...
        <c:if
          test="${sessionScope.loginUser.no == 1  || sessionScope.loginUser.no == question.memberNo}">
          ...
          <c:if test="${fn:length(answer.contents) == 0}">
            <a class="btn  btn-outline-dark"
              href='${contextRootPath}/app/question/update/${question.questionNo}'
      ...
      <c:if test="${sessionScope.loginUser.no == 1 }">
        <c:choose>
          <c:when test="${fn:length(answer.contents) == 0}">
            <button id="answer_Btn" class="btn  btn-outline-dark" onclick="button1_click();">답변하기</button>
          </c:when>
          <c:otherwise>
            <button class="btn  btn-outline-dark" id="answerModify_Btn" onclick="button2_click();">답변수정</button>
          </c:otherwise>
        </c:choose>
      </c:if>
    </div>
    ...
    <div id="answer_form" style="display: none;">
      ...
      <form id='add_form' action='add2' method='post' onsubmit="return add_check()">
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <textarea class="form-control col-sm-13 anwerContents summernote" rows="5"
    ...
    <div id="modify_form" style="display: none;">
      ...
      <form id='answer_modify_form' onsubmit="return add_Modify_check()" action='update2'
        method='post'>

        <div class="form-group row">
          <div class="col-sm-10" style="display: none;">
            <input type="text" class="form-control" name="answerNo" id="answerNo"
              value="${answer.answerNo}" />
          </div>
          <div class="col-sm-12">
            <textarea class="form-control col-sm-12 modify_contents summernote" rows="5"
              name="contents">${answer.contents}</textarea>
          </div>
        </div>
